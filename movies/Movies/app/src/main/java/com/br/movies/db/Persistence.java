...
public class Persistence {
    ...
    private List<String> tablesAutoIncrement;

    public Persistence(Context context) {
        this.context = context;
        this.tablesAutoIncrement = new ArrayList<>();
        createDatabase();
    }
    ...
    private void createTables() throws SQLException {
        ...
        for (Map.Entry<String, Map<String, String>> table : tables.entrySet()) {
            ...
            for(Map.Entry<String, String> column: table.getValue().entrySet()) {
                if (column.getKey().equals("auto")) {
                    tablesAutoIncrement.add(table.getKey());
                } else if (column.getKey().equals("pk")) {
    ...
    public void save(String table, Map<String, Object> value) throws SQLException {
        ...
        for (Map.Entry<String, String> column : columns.entrySet()) {
            if (column.getKey().equals("id") && tablesAutoIncrement.contains(table)) {
                int id = generateAutoIncrementId(table);
                values.put(column.getKey(), (Integer) id);
                continue;
            }
    ...
    public int generateAutoIncrementId(String table) {
        if (tablesAutoIncrement.contains(table)) {
            String query = Query.QUERY_AUTO_INCREMENT.concat(table);
